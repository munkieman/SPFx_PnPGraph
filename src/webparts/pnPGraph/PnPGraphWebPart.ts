...
import styles from './PnPGraphWebPart.module.scss';
...
export default class PnPGraphWebPart extends BaseClientSideWebPart<IPnPGraphWebPartProps> {
  ...
  public async render(): Promise<void> {
    ...
    <section class="${styles.pnPGraph} ${!!this.context.sdks.microsoftTeams ? styles.teams : ''}">
  ...
  private async getPermissions(): Promise<boolean> {
    ...
    try {
      ...
      graph.groups.getById("84278918-192b-4cae-a4a9-dc962f035575").expand("members")().then(async (group: { members: any; })=>{  
        ...
        for (let x = 0; x < group.members.length; x++) {
          console.log("member id",group.members[x].id);
          let groupMemberID = group.members[x].id;
